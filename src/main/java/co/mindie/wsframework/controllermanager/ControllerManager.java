...
import co.mindie.wsframework.controller.ControllerEntry;
import co.mindie.wsframework.controller.EndpointEntry;
...
import co.mindie.wsframework.utils.EndpointIndexer;

public class ControllerManager extends WSComponent {
	...
	public RequestHandler createRequestHandler(Class<?> controllerClass) {
		...
		if (entry == null) {
			throw new WSFrameworkException("Controller " + controllerClass + " was not loaded");
		}
	...
	private void createResponseWriter(EndpointEntry endpointEntry, RequestContext context) throws Exception {
		...
		if (responseWriterCls != void.class) {
			...
			if (!(responseWriter instanceof IResponseWriter)) {
				throw new WSFrameworkException("ResponseWriterClass does not implement IResponseWriter");
			}
		...
		if (context.getResponseWriter() == null) {
			...
			if (writer == null) {
				throw new WSFrameworkException("No compatible response writer found");
			}
	...
	private EndpointIndexer getEndpointIndexerForMethod(HttpMethod method) {
		...
		default:
			throw new WSFrameworkException("Unrecognized httpMethod: " + method);
		}
	}
		...
		if (shouldResolveOutput && response != null) {
			try {
				...
				if (resolverOutput != null) {
					...
					throw new WSFrameworkException("No resolver output found for " + response.getClass().getName());
				}
			} catch (Exception e) {
				thrownException = e;
